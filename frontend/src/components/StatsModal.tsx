...
export default function StatsModal({ stats, onClose }: Props) {
  ...
  const metrics = s
    ? [
        ...
        {
          label: "面试率",
          value: pct(s.interview_rate),
          sub: `进入面试 ${s.reached_interview}`,
        },
        {
          label: "Offer 率",
          value: pct(s.offer_rate),
          sub: `Offer ${s.offered}`,
        },
            ...
            <div className="grid grid-cols-4 gap-3">
              {metrics.map((m) => (
                ...
                >
                  ...
                  <div className="mt-1 text-2xl font-semibold font-mono text-slate-800">
                    {m.value}
                  </div>
              ...
              {s.by_resume_version.length === 0 ? (
                ...
                <div className="space-y-2.5">
                  {s.by_resume_version.map((v) => {
                    const w = Math.min(
                      100,
                      Math.max(4, v.interview_rate * 100),
                    );
                    return (
                      <div
                        key={v.resume_version}
                      ...
                      >
                        ...
                        <span className="w-24 shrink-0 text-right font-mono text-xs text-slate-500">
                          {v.reached_interview}/{v.total_applied} ·{" "}
                          {pct(v.interview_rate)}
                        </span>
                      </div>
                    );
                  })}
