...
import { buildInterviewMarkdown } from "../lib/markdown";
import type {
  Application,
  ApplicationInput,
  Interview,
...
export default function DetailModal({ app, onClose, onChanged }: Props) {
  ...
  useEffect(() => {
    setForm((f) =>
      f.status === app.status ? f : { ...f, status: app.status },
    );
  }, [app.status]);
  ...
  const deleteRecord = async () => {
    if (
      !confirm(`确认删除「${app.company} - ${app.position}」？此操作不可恢复。`)
    )
      return;
  ...
  const removeInterview = async (iv: Interview) => {
    if (
      !confirm(
        `确认删除面试「${iv.round_name || "未命名"}」？其中的问题记录也会一并删除。`,
      )
    )
      return;
  ...
  const inputCls =
    "w-full rounded-lg border border-slate-200 px-3 py-2 text-sm focus:border-amber-500 focus:ring-2 focus:ring-amber-200 focus:outline-none";
              ...
              return (
                ...
                >
                  ...
                  >
                    ...
                    <span
                      className={`h-2 w-2 rounded-full ${STATUS_META[iv.outcome === "PASSED" ? "OFFERED" : iv.outcome === "FAILED" ? "INTERVIEW_FAILED" : "APPLIED"].dot}`}
                    />
                    ...
                    <span
                      className={`rounded-full px-2 py-0.5 text-[11px] ${iv.outcome === "PASSED" ? "bg-emerald-50 text-emerald-600" : iv.outcome === "FAILED" ? "bg-rose-50 text-rose-600" : "bg-slate-100 text-slate-500"}`}
                    >
                      {iv.outcome === "PASSED"
                        ? "通过"
                        : iv.outcome === "FAILED"
...
function InterviewFormBody({
  ...
  // 临时面试 (尚未保存, id 为负): QA 依赖真实面试 id, 必须先保存
  const isTemp = initial.id < 0;
  ...
  const submit = async () => {
    ...
    try {
      await onSave({
        ...initial,
        ...draft,
        round_name: draft.round_name.trim() || "面试",
      });
      setMsg("已保存");
    } catch (e: any) {
  ...
  // 插入/删除后以后端为准重拉列表, 避免本地 sort_order 失真导致排序错乱
  const reloadQA = async () => {
    const fresh = await api.getApplication(initial.application_id);
    const iv = fresh.interviews?.find((i) => i.id === initial.id);
    setQaItems(iv?.qa_items ?? []);
  };
    ...
    <div
      className={
        fullscreen
          ...
          : "space-y-3 border-t border-slate-100 px-3 py-3"
      }
    >
      ...
      <div>
        <div className="mb-1.5 flex items-center justify-between">
          <label className={labelCls + " mb-0"}>
            逐条问题记录（{qaItems.length}）
          </label>
          <button
            type="button"
            onClick={() => setPreviewOpen(true)}
            disabled={isTemp}
            className="inline-flex items-center gap-1 rounded-md bg-slate-100 px-2 py-1 text-xs text-slate-600 hover:bg-slate-200 disabled:opacity-40 disabled:hover:bg-slate-100 transition-colors"
          >
            <FileText size={13} />
            Markdown 预览
          </button>
      ...
      {previewOpen && (
        <InterviewMarkdownModal
          iv={{ ...initial, qa_items: qaItems }}
...
// 面试 Markdown 预览弹窗
function InterviewMarkdownModal({
  iv,
  onClose,
}: {
  iv: Interview;
...
function QAItemRow({
  ...
  // 请求序号守卫: 快速连续失焦时, 只接受最后一次请求的返回, 避免旧响应覆盖新输入
  const seq = useRef(0);
  ...
  const save = async () => {
    ...
    try {
      ...
      if (cur === seq.current) setStatus("error");
    }
  };

  const inputCls =
    "w-full rounded-md border border-slate-200 bg-white px-2.5 py-1.5 text-xs focus:border-amber-500 focus:ring-2 focus:ring-amber-200 focus:outline-none";
  ...
  return (
    <div className="rounded-lg border border-slate-200 bg-slate-50/70 p-2.5">
      <div className="flex items-center justify-between mb-1.5">
        ...
        <div className="flex items-center gap-2">
          <span
            className={`text-[10px] ${status === "error" ? "text-rose-500" : status === "saved" ? "text-emerald-500" : status === "saving" ? "text-slate-400" : "text-transparent"}`}
          >
            {status === "saving"
              ? "保存中…"
              : status === "saved"
